...
export const sendVerificationEmail = async (email: string, token: string, firstName: string) => {
  ...
  const verificationLink = `${process.env.CLIENT_URL || 'https://verilens-frontend.pxxl.click'}/verify-email.html?token=${token}`;
...
export const sendPasswordResetEmail = async (email: string, token: string, firstName: string) => {
  const sendSmtpEmail = new SibApiV3Sdk.SendSmtpEmail();
  const resetLink = `${process.env.CLIENT_URL || 'https://verilens-frontend.pxxl.click'}/reset-password.html?token=${token}`;
